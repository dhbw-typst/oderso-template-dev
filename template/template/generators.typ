...
#let _acknowledgements-default-generator(config) = {
  ...
  align(center + horizon, {
    heading(outlined: false, numbering: none, [#text(
      0.85em,
      smallcaps(_linguify-content("acknowledgments")),
    )\ ])
    align(left, ack.text)
    v(20%)
  })
...
#let _abstracts-default-generator(config) = {
  ...
  for a in entries {
    ...
    align(center + horizon, {
      heading(outlined: false, numbering: none, [#text(
          0.85em,
          smallcaps(_linguify-content("abstract")),
        )\ #text(
          0.75em,
          weight: "light",
          style: "italic",
...
#let _toc-default-generator(config) = {
  // show level 1 headings in outline in a fancier way
  show outline.entry.where(level: 1): strong
  set par(leading: 0.65em)
  outline(
    title: _linguify-content("table-of-contents"),
    depth: 3,
    indent: auto,
...
#let _bibliography-default-generator(config) = {
...
/// configured. -> content | none
#let _abbreviations-default-generator(config) = {
  let abbr = config.front-back-matter.abbreviations
  let entries = abbr.at("entries", default: ())
  if entries.len() == 0 {
    return none
  }
  heading(_linguify-content("abbreviations"))
...
#let _listings-default-generator(config) = context {
  ...
  if (
    ...
    pagebreak(weak: true)
    heading(_linguify-content("list-of-figures"))
    outline(
  ...
  if (
    ...
    pagebreak(weak: true)
    heading(_linguify-content("list-of-tables"))
    outline(
  ...
  if (
    ...
    pagebreak(weak: true)
    heading(_linguify-content("list-of-code"))
    outline(
